...
if grep -Eqi 'replace-me|replace-with|change[_-]?me|placeholder' "$project_dir/.env"; then echo 'Refusing placeholder configuration.' >&2; exit 1; fi
...
(cd "$project_dir/frontend" && BROWSER=none PORT="${FRONTEND_PORT:?FRONTEND_PORT is required}" npm start) & frontend_pid=$!
